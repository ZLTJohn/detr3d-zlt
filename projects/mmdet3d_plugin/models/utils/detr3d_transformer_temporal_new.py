...
class Detr3DCrossAtten_Tx(BaseModule):
    def __init__(self,
                 embed_dims=256,
                 num_heads=8,
                 num_levels=4,
                 ...
                 temporal_len = 2,
                 mlp_fusion=False):
        super(Detr3DCrossAtten_Tx, self).__init__(init_cfg)
        ...
        self.attention_weights = nn.ModuleList([nn.Linear(embed_dims,
                                    num_cams*num_levels*num_points)for i in range(temporal_len)])
        self.output_proj = nn.Linear(embed_dims, embed_dims)

        if mlp_fusion == False:
            self.temporal_fusion_layer = nn.Linear(temporal_len*embed_dims, embed_dims)
        else:
            self.temporal_fusion_layer = mlp_residual(temporal_len*embed_dims, embed_dims)
    ...
    def forward(self,
        ...
        output_history = reference_points.new_tensor([])
        for i in range(len_queue):#actually len_queue=1 right now
            ...
            new_output = self.get_weight_feat(img_feats, query, reference_points, self.attention_weights[i], img_metas)
            output_history = torch.cat((output_history, new_output), -1)
            #refpt, img_feats, attn_weight the same
            #query fucked up
        reference_points_3d, output_now = self.get_weight_feat(value, query, reference_points, self.attention_weights[-1],\
